...
class Solution {
    ...
    static getScore(s) {
        ...
        try {
            let sLength = s.length; // Calculate the length of the string

            /** Iterate over the string up to the penultimate character. */
            for (let i = 0; i < (sLength - 1); i++) {
                let asciiValueFirst = s.charCodeAt(i);     // Get ASCII value of the current character.
                let asciiValueNext  = s.charCodeAt(i + 1); // Get ASCII value of the next character in the string.
